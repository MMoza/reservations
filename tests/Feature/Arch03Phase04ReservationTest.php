...
class Arch03Phase04ReservationTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function it_applies_early_booking_discount_for_30_days_advance()
    {
        ...
        $response = $this->postJson('/api/arch_03/v4/reservation', $payload);
    ...
    public function it_returns_reservation_with_all_phase04_fields_on_show()
    {
        ...
        $createResponse = $this->postJson('/api/arch_03/v4/reservation', $payload);
        $reservationId = $createResponse->json('id');

        $response = $this->getJson("/api/arch_03/v4/reservation/{$reservationId}");
